...
void main()
{
    ...
    if (!backgroundImage.loadFromFile("Data/background.jpg"))
        assert(0, "Error while loading background image.");
    Sprite background = new Sprite(backgroundImage);

    // Load the image needed for the wave effect
    Image WaveImage = new Image();
    if (!WaveImage.loadFromFile("Data/wave.jpg"))
        assert(0, "Error while loading wave image.");

    // Load all effects
    PostFX[char[]] Effects;
    foreach(char[] c; EFFECTS)
    {
        Effects[c] = new PostFX();
        if (!Effects[c].loadFromFile("Data/" ~ c ~ ".sfx"))
            assert(0);
    }
    ...
    Font f = new Font();
    if (!f.loadFromFile("Data/cheeseburger.ttf"))
        assert (0);
